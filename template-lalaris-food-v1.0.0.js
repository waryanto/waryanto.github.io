    ...
    if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(position => {
            ...
            userMarker.on('move', function(e) {
                ...
                // Hitung ulang jarak dan ongkos kirim
                const newDistance = calculateDistance(newLat, newLng, restoLat, restoLng);
                console.log("New distance in km:", newDistance);

                const newShippingCost = calculateShippingCost(newDistance);
                console.log("New shipping cost:", newShippingCost);

                // Update ongkos kirim di form
    ...
    fetch('/feeds/posts/default?alt=json')
        ...
        .then(data => {
            ...
            posts.forEach(post => {
				...
				// Parsing deskripsi produk
                const descriptionMatch = content.match(/<p>Deskripsi:<\/p>\s*<p>(.*?)<\/p>/s);
                const description = descriptionMatch ? descriptionMatch[1] : 'Deskripsi tidak tersedia.';


				// Parsing status produk
                const statusMatch = content.match(/<p>Status:\s*(.*?)<\/p>/);
                const status = statusMatch ? statusMatch[1].toLowerCase() : 'tersedia';
            ...
            for (const [category, products] of Object.entries(categorizedProducts)) {
                ...
                products.forEach(product => {
					...
					const isOutOfStock = product.status === 'habis'; // Periksa apakah produk habis
					const buttonText = isOutOfStock ? 'Habis' : 'Beli';
    				const buttonDisabled = isOutOfStock ? 'disabled' : '';

                    menuItem.innerHTML = `
                    <div class='menu-details'>
                        <h5 class='mb-1'>${product.title}</h5>
                        <span class='text-success mb-2 d-block'>Rp${formatNumberWithDot(product.price)}</span>
                        <p class='description mb-2'>${product.description}</p>
                    </div>
                    <div class='image-section d-flex flex-column align-items-center'>
                        ...
                        <button class='btn btn-primary buy-btn' data-price='${product.price}' data-title='${product.title}' data-toppings='${JSON.stringify(product.toppings)}' data-variations='${JSON.stringify(product.variations)}' ${buttonDisabled}>${buttonText}</button>
...
function showVariationModal(productName, variations, toppings) {
    ...
    const variationSection = variationSelect.closest('.form-group'); // Ambil section variasi
    const toppingContainer = document.getElementById('topping-options');
    const toppingSection = toppingContainer.closest('.form-group'); // Ambil section topping

    titleElem.textContent = `Pilih topping untuk ${productName}`;
    
    if (variations.length > 0) {
        variationSelect.innerHTML = ''; // Bersihkan variasi lama
        variations.forEach(variation => {
            const option = document.createElement('option');
            option.value = JSON.stringify(variation);
            option.textContent = `${variation.name} - Rp${formatNumberWithDot(variation.price)}`;
            variationSelect.appendChild(option);
        });
        variationSection.style.display = 'block'; // Tampilkan jika ada variasi
    } else {
        variationSection.style.display = 'none'; // Sembunyikan jika tidak ada variasi
    }
    ...
    if (toppings.length > 0) {
        ...
        toppings.forEach(topping => {
            ...
            toppingElem.innerHTML = `<input data-price='${topping.price}' type='checkbox' value='${topping.name}'/> ${topping.name} ${topping.price > 0 ? `- Rp${formatNumberWithDot(topping.price)}` : ''}`;

            toppingContainer.appendChild(toppingElem);
        });
        toppingSection.style.display = 'block'; // Tampilkan jika ada topping
    } else {
        toppingSection.style.display = 'none'; // Sembunyikan jika tidak ada topping
    }
...
document.getElementById('cart').addEventListener('click', function() {
            ...
            cartItem.innerHTML = `
                <div class='cart-item-details'>
                    ...
                    <div class='quantity-container'>
                        ...
                        <span class='item-price'>Rp${formatNumberWithDot(item.price * item.quantity)}</span>
    ...
    document.querySelectorAll('.quantity-btn').forEach(button => {
        button.addEventListener('click', function(even) {
			event.stopPropagation(); // Mencegah popup cart tertutup
    ...
    document.querySelectorAll('.quantity-input').forEach(input => {
        input.addEventListener('change', function() {
            ...
            if (newQuantity > 0) {
                ...
                this.value = cart[index].quantity; // Reset nilai ke nilai sebelumnya jika input tidak valid
            }
        });
    });

    // Update total pembayaran di popup
    const totalAmount = cart.reduce((total, item) => total + (item.price * item.quantity), 0);
    document.getElementById('order-total').textContent = `Rp${formatNumberWithDot(totalAmount)}`;
}
...
function checkout() {
    ...
    function updateTotalPayment(shippingCost) {
        ...
        document.getElementById('order-total').textContent = `Rp${formatNumberWithDot(orderTotal)}`;

        // Hitung total pembayaran
        const totalPayment = orderTotal + shippingCost;
		document.getElementById('total-payment-display').textContent = `Rp${formatNumberWithDot(totalPayment)}`;
    ...
    listItem.className = 'list-group-item d-flex justify-content-between align-items-start flex-wrap';
    listItem.innerHTML = `
	...
	//Subtotal
    orderTotalElement.textContent = `Rp${formatNumberWithDot(totalAmount)}`;
